...
import { X, Ship, Users, Car, Plus } from 'lucide-react';
import { useCreateFerry } from '@/hooks/useApi';

interface AddFerryModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  buttonText?: string;
...
const AddFerryModal: React.FC<AddFerryModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  ...
  const { createFerry, loading } = useCreateFerry();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ submit: error instanceof Error ? error.message : 'Failed to create ferry' });
    } finally {
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h2 className="text-xl font-semibold text-gray-900">Add New Ferry</h2>
                  <p className="text-sm text-gray-600 mt-1">
                    Create a new ferry for the fleet
                  </p>
                </div>
              </div>
              <button
